...
public class MySQLConnection {
    ...
    public MySQLConnection() throws MySQLException { // create connection
        try {
    ...
    public void setFavoriteItem(String userId, Item item) throws MySQLException {
        ...
        // Need to make sure item is added to the database first because of
        // the foreign key restriction on item_id(favorite_records) -> id(items)　
        saveItem(item);
        ...
        try {
            ...
            statement.setString(1, userId); // auto check the legitimacy of input
            statement.setString(2, item.getId()); // to prevent malicious attack
    ...
    public Set<String> getFavoriteItemIds(String userId) throws MySQLException {
        ...
        try {
            PreparedStatement statement = conn.prepareStatement(sql);
            statement.setString(1,userId);
            ResultSet result = statement.executeQuery(); // subset of the table according to your request
            while (result.next()) {
    ...
    public boolean addUser(User user) throws MySQLException {
        ...
        try {
            ...
            return statement.executeUpdate() == 1; // 1 means successfully inserted 1 column. 0 means duplicate.
        } catch (SQLException e) { // SQLException extends RuntimeException
            e.printStackTrace();
